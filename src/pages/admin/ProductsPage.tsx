...
const ProductsPage = () => {
  ...
  const getCategoryColor = (c: string) => ({ web: 'bg-blue-500/20 text-blue-400 border-blue-500/30', automation: 'bg-teal-500/20 text-teal-400 border-teal-500/30', dashboard: 'bg-green-500/20 text-green-400 border-green-500/30', branding: 'bg-pink-500/20 text-pink-400 border-pink-500/30', custom: 'bg-orange-500/20 text-orange-400 border-orange-500/30' }[c] || 'bg-gray-500/20 text-gray-400 border-gray-500/30');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={() => { activeTab === 'products' ? (setEditingProduct(null), setProductForm(emptyProduct), setError(''), setShowProductModal(true)) : (setEditingPackage(null), setPackageForm(emptyPackage), setError(''), setShowPackageModal(true)); }}
          className="flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-500 hover:to-cyan-500 text-white font-semibold rounded-xl transition-all">
      ...
      <div className="flex items-center space-x-4 border-b border-white/10">
        <button onClick={() => setActiveTab('products')} className={`px-6 py-3 font-semibold transition-all ${activeTab === 'products' ? 'text-teal-400 border-b-2 border-teal-400' : 'text-gray-400 hover:text-white'}`}>Products ({products.length})</button>
        <button onClick={() => setActiveTab('packages')} className={`px-6 py-3 font-semibold transition-all ${activeTab === 'packages' ? 'text-teal-400 border-b-2 border-teal-400' : 'text-gray-400 hover:text-white'}`}>Packages ({packages.length})</button>
      </div>

      <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
        <div className="mb-6 relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" /><input type="text" placeholder={`Search ${activeTab}...`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-teal-500" /></div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProducts.map((product) => (
              <motion.div key={product.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white/5 border border-white/10 rounded-xl p-6 hover:border-teal-500/30 transition-all">
                ...
                <div className="flex items-center justify-between mb-4"><div className="flex items-center space-x-2"><DollarSign className="w-4 h-4 text-teal-400" /><span className="text-white font-bold">£{product.base_price.toLocaleString()}</span></div><span className="text-xs text-gray-400 uppercase">{product.billing_type.replace('_', ' ')}</span></div>
                <div className="flex flex-wrap gap-2 mb-4">
                  {product.features.slice(0, 3).map((f, i) => <span key={i} className="px-2 py-1 bg-teal-500/10 text-teal-400 text-xs rounded-lg">{f}</span>)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPackages.map((pkg) => (
              <motion.div key={pkg.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`bg-white/5 border rounded-xl p-6 hover:border-teal-500/30 transition-all ${pkg.is_popular ? 'border-teal-500/50 ring-2 ring-teal-500/20' : 'border-white/10'}`}>
                {pkg.is_popular && <div className="mb-4"><span className="px-3 py-1 bg-gradient-to-r from-teal-600 to-cyan-600 text-white text-xs font-bold rounded-full">Most Popular</span></div>}
                ...
                <div className="space-y-2 mb-4">
                  {pkg.features.slice(0, 4).map((f, i) => <div key={i} className="flex items-start space-x-2"><CheckCircle className="w-4 h-4 text-teal-400 flex-shrink-0 mt-0.5" /><span className="text-xs text-gray-300">{f}</span></div>)}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-gradient-to-br from-gray-900 to-black border border-white/10 rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex justify-end space-x-4 mt-8 pt-6 border-t border-white/10">
                ...
                <button onClick={saveProduct} disabled={saving} className="px-6 py-3 bg-gradient-to-r from-teal-600 to-cyan-600 text-white rounded-xl disabled:opacity-50">{saving ? 'Saving...' : editingProduct ? 'Update' : 'Create'}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
        ...
        {showPackageModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-gradient-to-br from-gray-900 to-black border border-white/10 rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              {error && <div className="mb-4 p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex items-start space-x-3"><AlertCircle className="w-5 h-5 text-red-400" /><p className="text-red-400 text-sm">{error}</p></div>}
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-300 mb-2">Name *</label><input value={packageForm.name} onChange={(e) => setPackageForm({ ...packageForm, name: e.target.value, slug: slugify(e.target.value) })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-teal-500 focus:outline-none" /></div><div><label className="block text-sm font-medium text-gray-300 mb-2">Billing Cycle</label><select value={packageForm.billing_cycle} onChange={(e) => setPackageForm({ ...packageForm, billing_cycle: e.target.value as Package['billing_cycle'] })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-teal-500 focus:outline-none appearance-none">{(['monthly', 'annual', 'one_time'] as const).map((b) => <option key={b} value={b} className="bg-gray-900">{b.replace('_', ' ')}</option>)}</select></div></div>
              ...
              <div className="flex justify-end space-x-4 mt-8 pt-6 border-t border-white/10">
                ...
                <button onClick={savePackage} disabled={saving} className="px-6 py-3 bg-gradient-to-r from-teal-600 to-cyan-600 text-white rounded-xl disabled:opacity-50">{saving ? 'Saving...' : editingPackage ? 'Update' : 'Create'}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
